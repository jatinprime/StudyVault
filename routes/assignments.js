...
router.get('/upload', auth, (req, res) => {
  res.render('upload' , {category : 'assignments'});
});
...
router.get('/', auth, async (req, res) => {
  try {
    const files = await File.find({ user: req.user.id, category: 'assignments' });
    const existingFiles = [];
    ...
    res.render('files', { files: existingFiles, success, error , category : 'assignments' });
  } catch (err) {
    console.error(err.message);
