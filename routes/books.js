...
router.get('/upload', auth, (req, res) => {
  res.render('upload' , {category : 'books'});
});
...
router.get('/', auth, async (req, res) => {
  try {
    const files = await File.find({ user: req.user.id, category: 'books' });
    const existingFiles = [];
    ...
    res.render('files', { files: existingFiles, success, error , category : 'books' });
  } catch (err) {
    console.error(err.message);
